...
public:
    ...
    SDL_Texture* endMenuText=nullptr;
    ...
    Game() {
        ...
        SDL_Surface* endMenu = TTF_RenderText_Solid(font,"End game!",textColor);
        endMenuText = SDL_CreateTextureFromSurface(renderer,endMenu);
    ...
    while (SDL_PollEvent(&event)) {
        ...
        if (gameOver) {
             if (event.type == SDL_MOUSEBUTTONDOWN) {
                    ...
                    // Kiểm tra xem chuột có nhấp vào nút trở về menu không
                    SDL_Rect endMenuRect = {SCREEN_WIDTH / 2 - 50, SCREEN_HEIGHT / 2 + 40, 100, 40};
                    if (mouseX >= endMenuRect.x && mouseX <= endMenuRect.x + endMenuRect.w &&
                        mouseY >= endMenuRect.y && mouseY <= endMenuRect.y + endMenuRect.h) {
    ...
    void render() {
        ...
        if (gameOver) {
            ...
            SDL_Rect endMenuRect = {SCREEN_WIDTH / 2 - 50, SCREEN_HEIGHT / 2 + 40, 100, 40};
            SDL_RenderCopy(renderer, endMenuText, NULL, &endMenuRect);
        }
        else if (inMenu) {
            // Vẽ background menu
            //SDL_SetRenderDrawColor(renderer, 0, 0, 0, 255);
